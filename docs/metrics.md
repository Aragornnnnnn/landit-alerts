...
| 구독 중                     | RevenueCat. 월간·연간 결제는 `actives` 차트, 무료체험은 `trials` 차트(상품별). 프로모션과 해지예정은 아래 참고                                                                  |
...
연간 아래 첫 줄이다. 아직 쓸 수 있지만 다음 결제를 꺼 둔 구독 수로, 체험 중 해지도 여기 들어간다. 바로 위 연간 숫자에 이미 포함된 사람이라 더하는 줄이 아니다. 프로모션은 원래 갱신이 없어 세지 않는다.

구독 기록의 상품이 레비뉴캣 내부 번호(`prod…`)로만 들어 있어 월간·연간을 가를 수 없다. 그래서 이 값은 스토어 구독 전체를 센 것이고, 월간 구독자가 0명인 동안은 연간 해지예정과 같다. 상품이 늘면(월간, 할인가·정가) 키에 `project_configuration:products:read`를 더해 번호를 상품 이름으로 바꿔야 갈린다. 같은 이유로 지금은 할인가와 정가도 구분하지 못한다.
...
**버셀 크론이 주 경로다.** 버셀이 `api/metrics-cron`을 부르고, 그 함수가 깃허브에 "지금 돌려"라는 실행 요청만 보낸다. 일하는 곳은 그대로 깃허브 Actions다.
...
**시각 맞추기.** 버셀 무료 요금제는 적어 둔 분을 무시하고 그 시간대 끝에 돌린다. 실측으로 나흘 연속 UTC 0시 59분 49초에 돌았다. 그래서 크론을 UTC 23시(`0 23 * * *`)로 한 칸 앞당겨 KST 8시 59분쯤 깨우고, 워크플로 첫 단계에서 9시 정각까지 기다린다. 기다림은 길어도 한 시간이고 공개 레포라 실행 시간이 무료다. 예비책으로 늦게 도는 경우엔 이미 9시가 지나 그냥 통과한다.

분 단위로 더 정확히 하고 싶으면 외부 크론 서비스가 같은 요청을 보내게 하면 된다. 기다림도 사라진다.

워크플로에 남은 `schedule`(UTC 2시 12분)은 버셀이 실패했을 때의 예비책이다. 실측으로 다섯 시간까지 밀려 도착한다. 늦어도 상관없다 — 이미 보낸 기간이면 아무것도 하지 않는다.
...
- 새 알림 캠페인은 `lib.mjs`의 `CAMPAIGN_LABELS`에 이름을 더한다. 안 더해도 캠페인 이름 그대로 나간다. `admin_`으로 시작하는 어드민 공지는 캠페인 이름이 곧 문구라, 밑줄만 풀어서 그대로 보여 준다.
