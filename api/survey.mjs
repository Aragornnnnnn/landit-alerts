// 슈퍼베이스 Database Webhook 수신 엔드포인트 (Vercel 서버리스 함수) — 새 설문 응답을 디스코드로 보낸다
import { timingSafeEqual } from 'node:crypto';

import { sendEmbed } from '../src/shared/discord.mjs';
import { buildSurveyEmbed, parseSurveyWebhook } from '../src/survey/lib.mjs';
...
export default async (req, res) => {
  ...
  if (!isAuthorized(req.headers['x-survey-secret'])) {
    return res.status(401).json({ error: 'unauthorized' });
  }

  const record = parseSurveyWebhook(req.body);
  // 우리 관심사가 아닌 이벤트는 조용히 받아 준다 — 슈퍼베이스가 재시도하지 않게
  if (!record) return res.status(200).json({ skipped: true });

  await sendEmbed(process.env.DISCORD_WEBHOOK_SURVEY, buildSurveyEmbed(record));
